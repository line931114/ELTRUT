adminQnaDetail<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
...
<script type="text/javascript">
	...
	function button2_click(frm) {
		var theForm = document.frm;
		theForm.action = "adminQnaReplyDelete?CS_NUMBER=${qnaDetail.CS_NUMBER}";
	}
		...
		<div class="panel-body">
			<form:form id="frm" name="frm" action="frm" method="post" enctype="multipart/form-data">
				<div class="form-group">
					<label>QNA 글번호</label> 
					<input type="text" class="form-control" value="${qnaDetail.CS_NUMBER}" style="width: initial;" readonly />
				</div>

				<div class="form-group">
					<label>QNA 카테고리</label> 
					<input type="text" class="form-control" id="CS_CATEGORY" name="CS_CATEGORY" value="${qnaDetail.CS_CATEGORY}" style="width: initial;" readonly />
				</div>
				<div class="form-group">
					<label>제목</label> 
					<input type="text" class="form-control" id="CS_TITLE" name="CS_TITLE" value="${qnaDetail.CS_TITLE}" style="width: initial;" readonly />
				</div>
				...
				<div class="form-group">
					<label>내용</label>
					<textarea class="form-control" id="CS_CONTENT" name="CS_CONTENT" rows="10" cols="30" readonly>${qnaDetail.CS_CONTENT}</textarea>
				</div>
				<c:choose>
					<c:when test="${qnaDetail.CS_IMAGE1 != null}">
						<div class="form-group">
							<label>등록 이미지</label>
						</div>
						<div class="form-group">
							<img src="/ELTRUT/file/qnaFile/${qnaDetail.CS_IMAGE1}" alt="" onerror="this.style.display='none';" style="width: 650px;" />
						</div>
					</c:when>
				</c:choose>
				<div class="form-group">
					<label>문의날짜</label> 
					<input type="text" class="form-control" id="CS_REGDATE" name="CS_REGDATE"
						value="<fmt:formatDate value="${qnaDetail.CS_REGDATE}" pattern="YY.MM.dd HH:mm" />"
				...
				<c:if test="${qnaDetail.CS_REPCONTENT ne null}">
					<div class="form-group">
						<label>답변날짜</label> 
						<input type="text" class="form-control" id="CS_REGDATE" name="CS_REPDATE"
							value="<fmt:formatDate value="${qnaDetail.CS_REPDATE}" pattern="YY.MM.dd HH:mm" />"
							style="width: initial;" readonly />
					</div>
				</c:if>
				<c:if test="${qnaDetail.CS_REPCONTENT eq null}">
