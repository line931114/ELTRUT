...
import b21.spring.paging.AdminQNAPaging;
...
public class AdminQnaController {
	...
	public ModelAndView adminQnaList(CommandMap commandMap, HttpServletRequest request) throws Exception {
		...
		if (isSearch != null) {
			...
			if (searchNum == 0) {
				...
				commandMap.put("CS_TITLE", isSearch);
		...
		try {
			...
			if (categoryNum != null) {
				if (categoryNum == 1) {
					commandMap.put("CS_CATEGORY", "상품문의");
					adminQnaList = adminQnaService.qnaCategoryList(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (categoryNum == 2) {
					commandMap.put("CS_CATEGORY", "배송문의");
					adminQnaList = adminQnaService.qnaCategoryList(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (categoryNum == 3) {
					commandMap.put("CS_CATEGORY", "입금문의");
					adminQnaList = adminQnaService.qnaCategoryList(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (categoryNum == 4) {
					commandMap.put("CS_CATEGORY", "교환&반품 문의");
					adminQnaList = adminQnaService.qnaCategoryList(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (categoryNum == 5) {
					commandMap.put("CS_CATEGORY", "기타문의");
					adminQnaList = adminQnaService.qnaCategoryList(commandMap.getMap());
					totalCount = adminQnaList.size();
				}
		...
		try {
			...
			if (repState != null) {
				if (repState == 1) {
					commandMap.put("CS_REPSTATE", "답변대기");
					adminQnaList = adminQnaService.qnaRepState(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (repState == 2) {
					commandMap.put("CS_REPSTATE", "답변완료");
					adminQnaList = adminQnaService.qnaRepState(commandMap.getMap());
					totalCount = adminQnaList.size();
				} else if (repState == 3) {
					commandMap.put("CS_REPSTATE", "문의종료");
					adminQnaList = adminQnaService.qnaRepState(commandMap.getMap());
					totalCount = adminQnaList.size();
				}
			}
	...
	public ModelAndView adminQnaReplyForm(CommandMap commandMap, HttpServletRequest request) throws Exception {
		...
		mv.setViewName("/admin/qna/adminQNAReply");
		return mv;
	}
	...
	public ModelAndView adminQnaDelete(CommandMap commandMap, HttpServletRequest request) throws Exception {
		...
		if (request.getParameter("CS_IMAGE1") != null) {
			image1 = request.getParameter("CS_IMAGE1");
		...
		mv.setViewName("redirect:/adminQna/adminQnaList");
